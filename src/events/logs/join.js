...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (conchannel.enabled && conchannel.channelids.length > 0) {
            ...
            const pingtext = await config.embeds.SUCCESS(pingreplacing);

            for (const roleid of conchannel.addroles) {
                const server = await client.guilds.fetch(bot.serverid);
